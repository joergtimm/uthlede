...
filestoprod() {
    ...
    logdatei=deploy-$(date +%F_%H-%M-%S).log
   	...
   	rsync -avzP $currentdir/.??* --log-file="$logdatei"  --exclude-from=exclude-file.txt -e "ssh" ppptv:/home/ppptv/public_html
    rsync -avzP $currentdir/* --log-file=deploy.log --exclude-from=exclude-file.txt -e "ssh" ppptv:/home/ppptv/public_html
}
